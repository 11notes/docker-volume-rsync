      ...
      if [ -d "${2}/${3}" ]; then
        ...
        if [ -z "${RSYNC_LOCAL_SOURCE}" ]; then
          for HOST in ${SSH_HOSTS}; do
            ...
            nq -q /usr/bin/rsync -aze --mkpath --delete --rsh="ssh -p${SSH_PORT}" ${2}/${3}/ docker@${SSH_HOST}:${2}/${3}
          done
        else
        ...
        if [ -z "${RSYNC_LOCAL_SOURCE}" ]; then
          for HOST in ${SSH_HOSTS}; do
            ...
            nq -q /usr/bin/rsync -aze --mkpath --delete --rsh="ssh -p${SSH_PORT}" ${2}/ docker@${SSH_HOST}:${2}
          done 
        else
