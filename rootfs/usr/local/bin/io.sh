#!/bin/ash
  ...
  inotifyd_queue(){
    ...
    case "${1}" in
      ...
      *)
        INOTIFYD_PATH=$(echo "${2}" | sed -e "s@\/@_@g")
        INOTIFYD_FILE="${IODIR}/${INOTIFYD_PATH}"
        if [ -f "${INOTIFYD_FILE}" ]; then
          elevenLogJSON debug "queue busy for ${2}"
          return 1
        fi
        elevenLogJSON debug "queue created for ${2}"
        touch ${INOTIFYD_FILE}
      ;;
    esac
